# Conversion UX Model - qwen-pilot
...
| Primary buyer or user | Developer-tool builders and teams standardizing Qwen CLI agent usage. |
| Value wedge | Qwen-focused orchestration harness with typed workflows, prompts, and team coordination. |
...
- Optional entry: Internal automation harness ($1k-$5k setup) when this archived proof exactly matches the buyer problem.
- Revival step: Workflow template library ($8k-$25k rollout) only after the active successor and support boundary are clear.
- Longer-term path: Agent-ops setup engagement ($1k-$5k/month workflow maintenance) if the repository is refreshed and assigned an owner.

## Pricing Frame

- Do not price the archive as a maintained product.
- Price the revived diagnostic, workshop, template, or implementation path.
- Use Workflow success only as a discussion metric until fresh validation exists.

...
const HTTP_REQ: PromptBenchTool = {
  ...
  parameters: {
    ...
    properties: {
      ...
      method: { type: "string", enum: ["GET", "POST", "PUT", "DELETE", "PATCH"], description: "Method" },
...
const SET_CFG: PromptBenchTool = {
  ...
  parameters: {
    ...
    properties: {
      ...
      scope: { type: "string", enum: ["local", "global", "workspace"], description: "Scope" },
    },
...
export const PROMPT_BENCH_CASES: PromptBenchCase[] = [
  // Simple (5)
  {
    ...
    expected: [{ name: "read_file", arguments: { path: "/src/index.ts" } }],
  },
  {
    id: "s02",
    category: "simple",
    ...
    tools: [LIST_DIR],
    prompt: "List /src",
    expected: [{ name: "list_directory", arguments: { path: "/src" } }],
  },
  {
    id: "s03",
    category: "simple",
    ...
    tools: [RUN_CMD],
    prompt: "Run npm test",
    expected: [{ name: "run_command", arguments: { command: "npm test" } }],
  },
  {
    id: "s04",
    category: "simple",
    ...
    expected: [{ name: "search", arguments: { query: "TODO" } }],
  },
  {
    id: "s05",
    category: "simple",
    ...
    tools: [WRITE_FILE],
    prompt: "Write 'hello' to /tmp/hi.txt",
    expected: [{ name: "write_file", arguments: { path: "/tmp/hi.txt", content: "hello" } }],
  },
  ...
  // Type coercion (5)
  {
    ...
    tools: [SEARCH],
    prompt: "Case-sensitive search for 'Error' in /src",
    expected: [{ name: "search", arguments: { query: "Error", path: "/src", case_sensitive: true } }],
  },
  {
    id: "tc02",
    category: "type-coercion",
    description: "Integer not string",
    tools: [SEARCH],
    prompt: "Search 'import', max 5 results",
    expected: [{ name: "search", arguments: { query: "import", max_results: 5 } }],
  },
  {
    id: "tc03",
    category: "type-coercion",
    description: "Number not string",
    tools: [RUN_CMD],
    prompt: "Run 'npm build' with 30s timeout",
    expected: [{ name: "run_command", arguments: { command: "npm build", timeout: 30 } }],
  },
  {
    id: "tc04",
    category: "type-coercion",
    description: "Enum exact match",
    tools: [HTTP_REQ],
    prompt: "POST to https://api.example.com/data",
    expected: [{ name: "http_request", arguments: { url: "https://api.example.com/data", method: "POST" } }],
  },
  {
    id: "tc05",
    category: "type-coercion",
    ...
    tools: [CREATE_PR],
    prompt: "Non-draft PR 'Fix bug' from fix/bug to main",
    expected: [
      { name: "create_pull_request", arguments: { title: "Fix bug", base: "main", head: "fix/bug", draft: false } },
    ],
  },
  ...
  // Multi-param (5)
  {
    ...
    description: "Required + all optional",
    tools: [SEARCH],
    prompt: "Case-insensitive search for 'config' in /etc, max 10",
    expected: [
      { name: "search", arguments: { query: "config", path: "/etc", case_sensitive: false, max_results: 10 } },
    ],
  },
  {
    id: "mp02",
    ...
    tools: [CREATE_PR],
    prompt: "PR 'Add feat' from feat/x to main, labels: bug, enhancement",
    expected: [
      {
        name: "create_pull_request",
        arguments: { title: "Add feat", base: "main", head: "feat/x", labels: ["bug", "enhancement"] },
      },
    ],
  },
  {
    id: "mp03",
    category: "multi-param",
    description: "Enum + number + string",
    tools: [HTTP_REQ],
    prompt: "DELETE https://api.example.com/item/42 timeout 5000ms",
    expected: [
      { name: "http_request", arguments: { url: "https://api.example.com/item/42", method: "DELETE", timeout: 5000 } },
    ],
  },
  {
    id: "mp04",
    category: "multi-param",
    description: "Config with enum scope",
    tools: [SET_CFG],
    prompt: "Set 'theme' to 'dark' at workspace scope",
    expected: [{ name: "set_config", arguments: { key: "theme", value: "dark", scope: "workspace" } }],
  },
  {
    id: "mp05",
    category: "multi-param",
    description: "Command with dir and timeout",
    tools: [RUN_CMD],
    prompt: "Run 'make install' in /opt/project, 120s timeout",
    expected: [
      { name: "run_command", arguments: { command: "make install", timeout: 120, working_dir: "/opt/project" } },
    ],
  },
  ...
  // Multi-tool (5)
  {
    id: "mt01",
    category: "multi-tool",
    description: "Two parallel reads",
    tools: [READ_FILE],
    prompt: "Read /src/main.ts and /src/utils.ts",
    expected: [
  ...
  },
  {
    id: "mt02",
    category: "multi-tool",
    description: "Search + list parallel",
    tools: [SEARCH, LIST_DIR],
    prompt: "Search 'export' in /src and list /src",
    expected: [
      { name: "search", arguments: { query: "export", path: "/src" } },
  ...
  },
  {
    id: "mt03",
    category: "multi-tool",
    description: "Read then write (sequential)",
    tools: [READ_FILE, WRITE_FILE],
    prompt: "Read /config.json then write '{}' to /config.bak.json",
    expected: [
      { name: "read_file", arguments: { path: "/config.json" } },
      { name: "write_file", arguments: { path: "/config.bak.json", content: "{}" } },
    ],
  },
  {
    id: "mt04",
    category: "multi-tool",
    description: "Three parallel reads",
    tools: [READ_FILE],
    prompt: "Read /a.ts, /b.ts, and /c.ts",
    expected: [
  ...
  },
  {
    id: "mt05",
    category: "multi-tool",
    description: "Command + search parallel",
    tools: [RUN_CMD, SEARCH],
    prompt: "Run 'git status' and search for 'FIXME'",
    expected: [
      { name: "run_command", arguments: { command: "git status" } },
      { name: "search", arguments: { query: "FIXME" } },
    ],
  },
];
  ...
  return tc.expected.map((e, idx) => {
    ...
    switch (tc.category) {
      ...
      case "multi-param": {
        ...
        if (keys.length > 2 && lastKey) delete args[lastKey];
        break;
      }
      case "multi-tool":
...
export function formatPromptBenchTable(summary: PromptBenchSummary): string {
  ...
  for (const [cat, stats] of Object.entries(summary.byCategory)) {
    ...
    lines.push(
      `  ${cat.padEnd(catCols.cat)}  ${wp.padEnd(catCols.with)}  ${np.padEnd(catCols.without)}  ${String(stats.total).padEnd(catCols.total)}`,
    );
  }

  lines.push("");
  lines.push(
    `  With prompt:    ${summary.withPrompt.accuracy.toFixed(1)}% (${summary.withPrompt.passed} calls passed)`,
  );
  lines.push(
    `  Without prompt: ${summary.withoutPrompt.accuracy.toFixed(1)}% (${summary.withoutPrompt.passed} calls passed)`,
  );
